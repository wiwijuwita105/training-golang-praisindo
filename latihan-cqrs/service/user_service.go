...
type IUserService interface {
	...
	PostCreateUser(ctx context.Context, user *entity.User) (string, error)
}
...
func sendKafka(producer sarama.SyncProducer, ctx context.Context, user entity.User) error {
	...
	msg := &sarama.ProducerMessage{
		...
		Key:   sarama.StringEncoder(user.Name + "_" + user.Email + "_" + user.Password),
...
func (s *userService) PostCreateUser(ctx context.Context, user *entity.User) (string, error) {
	...
	err = sendKafka(producer, ctx, *user)
	if err != nil {
		return "", err
	}

	return "Create user in progress.", nil
}
