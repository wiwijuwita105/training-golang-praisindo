...
import (
	...
	"fmt"
	"log"
	"session9-crud-user-grpc/entity"
...
func (u *UserHandler) UpdateUser(ctx context.Context, req *pb.UpdateUserRequest) (*pb.MutationResponse, error) {
	_, err := u.userService.GetUserByID(ctx, int(req.GetId()))
	if err != nil {
		log.Println(err)
		return nil, err
	}
	...
	rss, err := u.userService.UpdateUser(ctx, int(req.GetId()), user)
	if err != nil {
		log.Println(err)
		return nil, err
	}
	...
	return &pb.MutationResponse{
		Message: fmt.Sprintf("Success update user with ID %d", rss.ID),
	}, nil
}
