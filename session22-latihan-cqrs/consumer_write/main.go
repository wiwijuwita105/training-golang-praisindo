...
type ConsumerInsert struct{}

func (ConsumerInsert) Setup(_ sarama.ConsumerGroupSession) error { return nil }

func (ConsumerInsert) Cleanup(_ sarama.ConsumerGroupSession) error { return nil }

func (h ConsumerInsert) ConsumeClaim(sess sarama.ConsumerGroupSession, claim sarama.ConsumerGroupClaim) error {
...
func main() {
	var err error
	db, err = gorm.Open(postgres.Open(config.DBWriteConn), &gorm.Config{SkipDefaultTransaction: true})
	if err != nil {
		slog.Error("error opening write database", slog.String("dsn", config.DBWriteConn), slog.Any("err", err))
		os.Exit(1)
	}

	log.Println("starting migration in db-write")
	if err := db.AutoMigrate(entity.User{}); err != nil {
		log.Println(err)
	}
	log.Println("finish migration in db-write")
